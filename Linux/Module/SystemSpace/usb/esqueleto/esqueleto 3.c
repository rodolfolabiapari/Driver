...
static ssize_t skel_write(struct file *file, const char __user *user_buffer, size_t count, loff_t *ppos)
{
	...
	printk(KERN_ERR "|- - - - - - - - Tentando desativar as interrupcoes\n");
	if (down_interruptible(&dev->sem)) {
		retval = -ERESTARTSYS;
		printk(KERN_ERR "|- - - - - - - - Erro ao tentar desativar as interrupcoes\n");
		goto exit;
	}
	printk(KERN_ERR "|- - - - - - - - Interrupcoes desativadas com sucesso para realizacao do write()\n");

	// Verify that the device wasn't unplugged. 
	if (! dev->udev) {
		retval = -ENODEV;
		printk(KERN_ERR "|- - - - - - - - No device or device unplugged (%d)\n", retval);
		goto unlock_exit;
	}
	printk(KERN_ERR "|- - - - - - - - Dispositivo USB ainda plugado no computador\n");
	...
	printk(KERN_ERR "|- - - - - - - - Por padrao, este driver enviara' apenas 1 caractere\n");
	...
	printk(KERN_ERR "|- - - - - - - - Bloqueando as treads\n");
	spin_lock(&dev->cmd_spinlock);
	printk(KERN_ERR "|- - - - - - - - Treads bloqueadas com sucesso\n");

		printk(KERN_ERR "|- - - - - - - - Alterando a variavel 'command' da estrutura geral\n");
		dev->command = *buf;

	spin_unlock(&dev->cmd_spinlock);
	printk(KERN_ERR "|- - - - - - - - Threads desbloqueadas com sucesso\n");
	...
	printk(KERN_ERR "|- - - - - - - - Ativando interrupcoes...    ");
	up(&dev->sem);
	printk(KERN_ERR "|- - - - - - - - Interrupcoes ativadas com sucesso\n");
...
static int skel_probe(struct usb_interface *interface, const struct usb_device_id *id)
{
	...
	sema_init(&dev->sem, 1);
	printk(KERN_INFO "|- - - - - - Semaforo Iniciado. Valor maximo de acessos simul e' igual a 1\n"); 
	// lock para as threads
	spin_lock_init(&dev->cmd_spinlock);
	printk(KERN_INFO "|- - - - - - Lock para treads iniciado com sucesso\n"); 
