...
int kiev_init(void) {
	...
	if (usb_register(&conversor_driver) != 0) {
		printk("<Alerta>    |----USB: Registro não realizado.\n"); 
	}
...
static int conversor_probe(struct usb_interface *interface, const struct usb_device_id *id)
{
	printk(KERN_INFO "<Alerta>    Dispositivo permitido para conexão (%04X:%04X) foi conectado.\n", id->idVendor, id->idProduct);
	return 0;
}
...
static void conversor_disconnect(struct usb_interface *interface)
{
	printk(KERN_INFO "<Alerta>    Dispositivo permitido para conexão (%04X:%04X) foi removido.\n", id->idVendor, id->idProduct);
}
